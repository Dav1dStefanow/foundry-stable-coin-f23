 ...
 *
 * So if the Chainlink network explodes and you have a lot of money locked in a protocol... too bad.
 */
library OracleLib {
    ...
    {
        (uint80 roundId, int256 answer, uint256 startedAt, uint256 updatedAt, uint80 answeredIdRound) =
            priceFeed.latestRoundData();
        uint256 secondsSince = block.timestamp - updatedAt;
        if (secondsSince > TIMEOUT) revert OracleLib__StalePrice();
        
        return (roundId, answer, startedAt, updatedAt, answeredIdRound);
    }
}
